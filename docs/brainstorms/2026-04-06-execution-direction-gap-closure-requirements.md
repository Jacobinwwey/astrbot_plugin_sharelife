...
| R4 | Partial (advancing) | Shared runtime helpers (`sharelife/webui/runtime_helpers.js`), market filter helpers (`sharelife/webui/market_filters.js`), facet view helper (`sharelife/webui/market_facet_view.js`), and event-binding helper (`sharelife/webui/market_event_bindings.js`) now back `market_page.js`; coverage in dedicated webui tests and meta wiring tests | Primary orchestration files are still large (`sharelife/webui/app.js`, `sharelife/webui/market_page.js`), so further extraction remains needed for lower coupling. |
