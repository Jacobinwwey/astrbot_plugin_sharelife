...
type SidebarMap = Record<string, SidebarGroup[]>

const base = process.env.DOCS_BASE || '/astrbot_plugin_sharelife/'
const docsRoot = path.resolve(__dirname, '..')
const hasPrivateDocsPortal = fs.existsSync(path.resolve(docsRoot, 'private', 'index.md'))
const privateSidebarPath = path.resolve(__dirname, 'private-sidebar.json')
const privateSidebar: SidebarMap = fs.existsSync(privateSidebarPath)
  ? (JSON.parse(fs.readFileSync(privateSidebarPath, 'utf-8')) as SidebarMap)
  : {}
...
if (hasPrivateDocsPortal) {
  nav.push({ text: 'Internal', link: '/private/' })
}

const publicSidebar: SidebarMap = {
  '/zh/': [
    {
