...
4. 插件安装执行默认关闭，必须管理员确认并通过命令前缀白名单。
...
10. [用户面板与市场页重构执行方案](/zh/reference/user-panel-stitch-execution-plan)
11. [存储持久化与冷备执行方案](/zh/reference/storage-cold-backup-execution-plan)
12. [集成执行手册（UI x 存储）](/zh/reference/integrated-execution-playbook)
13. [Sharelife v1 冻结方案](/zh/reference/sharelife-v1-freeze)
14. [API v1 参考](/zh/reference/api-v1)
15. [为什么社区优先](/zh/explanation/community-first)

## 私有运维边界

Reviewer 准入、Admin 运维 Runbook、可观测性值班手册、本地鉴权备份流程不再出现在公开文档站点。
这些内容应仅保存在本地 `docs-private/` 或独立内网仓库中。
